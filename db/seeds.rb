if Rails.env.development?
  ...
  mercenaries.each_with_index do |mercenary_data, index|
    begin
      ...
      # Créer le mercenaire sans photo_url pour l'instant (on utilisera Active Storage)
      mercenary = Mercenary.new(mercenary_data.except(:photo_url))

      # Attacher l'image à Active Storage
      mercenary.picture.attach(
        io: downloaded_image,
        filename: "#{mercenary_data[:name].parameterize}.jpg", # Nom du fichier
        content_type: "image/jpeg" # Type MIME (ajuste si nécessaire, ex: image/png)
      )

      # Sauvegarder le mercenaire
      mercenary.save!
      puts "Mercenaire #{index + 1} (#{mercenary_data[:name]}) créé avec succès ! Image attachée."
    rescue StandardError => e
  ...
  bookings = [
    # John Doe réserve John Rambo
    {
      user_id: users[0].id,
      mercenary_id: Mercenary.find_by(name: "John Rambo").id,
      start_date: Date.today,
      end_date: Date.today + 3.days,
      total_price: (3 * 1200), # 3 jours * 1200€/jour
      mission_purpose: "Extraction dans une zone de guerre",
      mission_place: "Jungle du Vietnam"
    },
    # John Doe réserve Wonder Woman
    {
      user_id: users[0].id,
      mercenary_id: Mercenary.find_by(name: "Wonder Woman").id,
      start_date: Date.today + 5.days,
      end_date: Date.today + 7.days,
      total_price: (2 * 2000), # 2 jours * 2000€/jour
      mission_purpose: "Récupération d’un artefact sacré",
      mission_place: "Ruines de Themyscira"
    },
    # Jane Smith réserve Ellen Ripley
    {
      user_id: users[1].id,
      mercenary_id: Mercenary.find_by(name: "Ellen Ripley").id,
      start_date: Date.today + 1.day,
      end_date: Date.today + 4.days,
      total_price: (3 * 850), # 3 jours * 850€/jour
      mission_purpose: "Élimination d’une infestation extraterrestre",
      mission_place: "Station spatiale abandonnée"
    },
    # Mike Wilson réserve Ethan Hunt
    {
      user_id: users[2].id,
      mercenary_id: Mercenary.find_by(name: "Ethan Hunt").id,
      start_date: Date.today - 2.days,
      end_date: Date.today + 1.day,
      total_price: (3 * 1100), # 3 jours * 1100€/jour
      mission_purpose: "Infiltration d’un réseau criminel",
      mission_place: "Berlin, Allemagne"
    }
  ]

  # Sauvegarde des bookings avec gestion des erreurs
  bookings.each_with_index do |booking_data, index|
    begin
      Booking.create!(booking_data)
      puts "Booking #{index + 1} créé avec succès !"
    rescue StandardError => e
